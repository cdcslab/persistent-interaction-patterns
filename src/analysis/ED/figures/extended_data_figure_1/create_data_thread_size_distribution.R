...
for (social_media_name in social_to_work_with) {
  ...
  # Compute distribution of number of thread sizes
  
  tmp = df[,.(thread_size = .N), 
           by = .(topic,root_submission)]
  
  # Count each number
  
  tmp = tmp[,.(count = .N), 
            by = .(topic,thread_size)]
  
  # Aggregate with other data
  
  tmp$social = ifelse(social_media_name == "facebook_news","Facebook",str_to_title(social_media_name))
...
if (identical(social_to_work_with,
  ...
  write_parquet(all_data, "data/Results/extended_data_figure_1/data_for_plot_thread_size_distribution_except_fb_news.parquet") 
  ...
  all_data_old = read_parquet("data/Results/extended_data_figure_1/data_for_plot_thread_size_distribution_except_fb_news.parquet") %>%
